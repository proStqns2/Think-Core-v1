...
const InternetAnimation: React.FC = () => {
    ...
    for (let a = 0; a < particles.current.length; a++) {
      for (let b = a; b < particles.current.length; b++) {
        ...
        if (distance < (120 * 120)) {
          ...
          const aColor = particles.current[a].color.match(/hsl\((\d+),/);
          const bColor = particles.current[b].color.match(/hsl\((\d+),/);
          if (aColor && bColor) {
            const grad = ctx.createLinearGradient(particles.current[a].x, particles.current[a].y, particles.current[b].x, particles.current[b].y);
            grad.addColorStop(0, `hsla(${aColor[1]}, 70%, 60%, ${opacityValue})`);
            grad.addColorStop(1, `hsla(${bColor[1]}, 70%, 60%, ${opacityValue})`);
  ...
  return <canvas ref={canvasRef} style={{ display: 'block', background: '#000510' }} />;
};
