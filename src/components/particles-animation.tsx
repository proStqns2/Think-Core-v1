...
const ParticlesAnimation: React.FC<ParticlesAnimationProps> = ({
  ...
  const setupAnimation = useCallback(() => {
    ...
    const update = (t: number) => {
      ...
      lastTime = t;
      elapsed += delta * (speed || 0.1);

      program.uniforms.uTime.value = elapsed * 0.001;

      if (moveParticlesOnHover) {
        particles.position.x = -mouseRef.current.x * (particleHoverFactor || 1);
        particles.position.y = -mouseRef.current.y * (particleHoverFactor || 1);
      } else {
      ...
      if (!disableRotation) {
        ...
        particles.rotation.z += 0.01 * (speed || 0.1);
      }
